...
import { GenerateSixthHospital } from '@/service/api/configureTask';

defineOptions({
  name: 'SixthHospitalGenerating'
});
...
const callGenerateAPI = async () => {
  try {
    const imageData = route.query.imageData;
    if (!imageData) {
    ...
    console.log('解析的图片数据:', parsedImageData);
